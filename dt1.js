...
legend.selectAll("g")
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        const color = d3.scaleOrdinal(d3.schemeSet3); // Predefined soft color palette
        ...
        d3.csv("dataset/OECD_Australia_Water_Use2015-2022.csv").then(csvData => {
            // Ensure numerical conversion for OBS_VALUE
            ...
            // Checkbox handling
            d3.selectAll("input[type='checkbox']").on("change", filterData);
        });
        ...
        function filterData() {
            ...
            // Default behavior if no categories are selected
            if (selectedCategories.length === 0) {
                updateData([]); // Show an empty pie chart
                return;
            }
    
            // Filter the data based on selected year and categories
            const filteredData = rawData
                .filter(
                    d =>
                        d.year === selectedYear &&
                        selectedCategories.includes(d.Measure)
                )
                .map(d => ({
                    category: d.Measure,
                    value: d.value,
        ...
        function updateChart() {
            ...
            const slices = chartGroup
                ...
                .on("mouseover", (event, d) => {
                    tooltip
                        ...
                        .html(
                            `<strong>${d.data.category}</strong><br>Value: ${d.data.value} Gigaliters`
                        )
           ...
           // Add transition for smoother updates
            slices.transition().duration(500).attrTween("d", function(d) {
